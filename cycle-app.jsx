...
const todayBase = new Date(2026, 4, 16); // May 16, 2026 — frozen "today"
...
function relDays(target, base = todayBase) {
...
// ─── main app ──────────────────────────────────────────────────────────────
function CycleApp({ scenario, dark, calSyncInit, accent, accentDeep, accentMuted, font }) {
  ...
  const [periods, setPeriods] = useState(() => scenarioPeriods(scenario));
  useEffect(() => { setPeriods(scenarioPeriods(scenario)); }, [scenario]);

  const [cycleLen, setCycleLen] = useState(27);
  const [cycleMode, setCycleMode] = useState('manual');
  const [periodLen, setPeriodLen] = useState(5);
  const [periodMode, setPeriodMode] = useState('manual');
